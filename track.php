...
<!-- Word:  <?php echo $_POST["Word"]; ?><br> -->
<?php
    echo "<a class='click-me' href='http://45.36.146.185/~willpar/PHPdemo.php'>Go Back</a>";
    if ($query = $connection->prepare("SELECT Artist, Title, Word, Word_Count FROM Recordings natural join Words natural join Word_Count where Title = ? ORDER BY ARTIST ASC")) {
        $query->bind_param("s", $_POST['Track']);
                $query->execute();
                $result = $query->get_result();
                $query->close();          
    }
//     if ($query = $connection->prepare("SELECT Artist, Title, Word, Word_Count FROM Recordings natural join Words natural join Word_Count where Word = ? ORDER BY ARTIST ASC")) {
//         $query->bind_param("s", $_POST['Word']);
//         $query->execute();
//         $result = $query->get_result();
//         $query->close();
// }
